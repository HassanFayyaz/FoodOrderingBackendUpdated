...
public interface RequestForProductRepository extends JpaRepository<RequestForProduct,Long> {
    ...
    @Query(value = "select id,count,date1,name from foodorderingdb.request_for_product order by count desc limit 5",nativeQuery = true)
...
//    public List<RequestForProductDTO> topRequestedProductsKeyword(@Param("keyword")String keyword);
    ...
    @Query(value = "SELECT *  from foodorderingdb.request_for_product WHERE name LIKE %:keyword%", nativeQuery = true)
